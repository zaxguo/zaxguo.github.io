---
layout: home
comments: false
---

I am now a tenure-track Assistant Professor at University of Electronic Science and Technology of China (UESTC) in the school of Computer Science. 
My research is in the intersection of OS x Arch x PL, under the umbrella of efficiency and security.   
Prior to UESTC, I was lucky to be awarded [Top Minds](https://career.huawei.com/reccampportal/portal5/topminds.html) from Huawei, where I had a little dabble in industry. 

I received my doctoral degree in 2022 from University of Virginia, advised by Prof. [Felix Xiaozhu Lin](https://fxlin.github.io) and bachelor degree in 2016 from Sun Yat-sen University, advised by Prof. [Zibin Zheng](https://www.zibinzheng.com/).
Prior to UVA, I have spent amazing four years at Purdue University with awesome folks of [XSEL](http://xsel.rocks) in the School of Electric and Computer Engineering. 

My past and ongoing projects span multiple aspects of **kernel**, including **drivers** that provide energy isolation, **file systems** that are highly secure on IoT devices, to **dynamic binary translation** that run a wimpy peripheral cores.

---



---

# Publication

* [**USENIX ATC'24** *(to appear)* ] "An Empirical Study of Rust-for-Linux: The Success, Dissatisfaction, and Compromise", **Hongyu Li**, **Liwei Guo**, Yexuan Yang, Shangguang Wang, Mengwei Xu. (also invited to appear on **USENIX ;login:**)
* [**ASPLOS'23**] "Efficient NLP Inference at the Edge via Elastic Pipelining", **Liwei Guo**, Wonkyo Choe, and Felix Xiaozhu Lin.
* [**EuroSys'22**] "Minimum Viable Device Drivers for ARM TrustZone", **Liwei Guo**, and Felix Xiaozhu Lin, in Proc. ACM European Conference on Computer Systems 2022. [code](https://zaxguo.github.io/drvlet/index.html)
* [**USENIX ATC’19**] "Transkernel: An Executor for Commodity Kernels on Peripheral Cores", **Liwei Guo**, Shuang Zhai, Yi Qiao, and Felix Xiaozhu Lin, in Proc. USENIX Annual Technical Conference 2019. [code](https://thexsel.github.io/p/transkernel/index.html)
* [**EuroSys’18**] "Power Sandbox: Power Awareness Redefined", **Liwei Guo**, **Tiantu Xu**, Mengwei Xu, Xuanzhe Liu, and Felix Xiaozhu Lin, in Proc. ACM European Conference on Computer Systems 2018. (co-primary) [code](https://thexsel.github.io/p/psbox/index.html)
* [**HotMobile’17**] "Decelerating Suspend and Resume in OS", Shuang Zhai, **Liwei Guo**, Xiangyu Li, and Felix Xiaozhu Lin, in Proc. ACM Int. Workshop on Mobile Computing Systems and Applications, 2017.
* [Arxiv](https://arxiv.org/abs/2205.10963) "Protecting File Activities viai Deception for ARM TrustZone", **Liwei Guo**, Kaiyang Zhao, Yiying Zhang, and Felix Xiaozhu Lin.


---

# A Little Bit More
Besides research, I mostly enjoy:
*	Playing video games which ignited my original passion for computers since I was six
(Diamond in [Legue of Legends](), Platinum in [Overwatch](), Divine in [Dota2]()).
Check out my game clips [here](placeholder)!
*	Photography. I record my life by shooting photos. Check out my (*awesome*) photos [here](gallery/photos)!

---

